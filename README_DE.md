...
>
> **Windows-Testbuild (offen):** Branch [`windows-wasapi-fix`](https://github.com/knoelliX/NativMix/tree/windows-wasapi-fix) mit Fixes für Other Apps, Mute-Zuverlässigkeit und App-Namen (z. B. RE-KORD). Installer über [Actions → Build Windows Installer](https://github.com/knoelliX/NativMix/actions/workflows/build-windows.yml) (manuell starten → Artifact laden). Kein Release-Tag — **jeder darf testen und Feedback geben**, auch Kleinigkeiten. Der Maintainer nutzt Windows nicht täglich, deshalb helfen eure Berichte besonders.
